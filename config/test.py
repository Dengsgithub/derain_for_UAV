...
from drd_net import detail_net
from dataset import TrainDataset, TestDataset
from cal_ssim import SSIM
from skimage.measure import compare_psnr as psnr2


def get_args():
    parser = argparse.ArgumentParser(description="train derain model",
                                     formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument("--train_dir", type=str, default='../dataset/Rain_200_H/train',
                        help="train image dir")
    parser.add_argument("--test_dir", type=str, default='../dataset/Rain_200_H/val',
                        help="test image dir")
    ...
    parser.add_argument("--sessname", type=str, default="nuaa_no_recursive",
...
class Session:
    def __init__(self, args):
        ...
        self.sche = MultiStepLR(self.opt, milestones=[30, 60, 90, 120, 150, 180, 210], gamma=0.5)
...
def run_train_val(args):
    sess = Session(args)
    sess.load_checkpoints("D:\\Desktop\\new_code\\models\\nuaa_no_recursive\\epoch 211_ssim 0.767073")
    file_name = "../psnr.txt"
    file = "../dataset/Rain_200_H/val"
    ...
    with open(file_name, 'a') as f:
        for image_file in image_files:
            ...
            B = image[:, 0:ww//2, :]
            O = image[:, ww//2:, :]
            O = np.transpose(np.expand_dims(O, axis=0), (2, 0, 1))
            B = np.transpose(np.expand_dims(B, axis=0), (2, 0, 1))
            sample = {'OB': O, 'GT': B}
            result_val, loss, ssim = sess.inf_batch("val", sample)
            psnr = psnr2(image[:, 0:ww//2, :], result_val.cpu())
            all_psnr.append(psnr)
            f.write(str(image_file).split("\\")[-1] + "," + str(psnr) + "," + str(ssim) + "\n")
        f.write("all," + str(np.mean(all_psnr)) + "\n")
